...
{
	// assert(s != 0);
	void* c = malloc_fun == my_malloc? malloc(s): malloc_fun(s) ;
	if(c == NULL)
		fprintf(stderr, "%s\n", nomem_err);
	return c;
}
...
{
	// assert(s != 0);
	void* c = realloc_fun == my_realloc? realloc(p, s): realloc_fun(p, s);
	if(c == NULL)
		fprintf(stderr, "%s\n", nomem_err),
		c = p;
	return c;
...
void my_free(void* p)
{
	free_fun == my_free? free(p): free_fun(p);
}
...
void* (*malloc_fun)(size_t size) = my_malloc;
void* (*realloc_fun)(void *b, size_t bytes) = my_realloc;
void (*free_fun)(void *ptr) = my_free;
...
/*
Funcion Hash para cadenas, Inspirada en la funcion:
...
void rehash_table(strset_t* set)
{
	...
	for( ; i<n; ++i )
	{
		...
		{
			str_t* sptr = (*j)->obj;
			size_t hash = Hash(arr(sptr), get_len(sptr)), k = hash%n;
...
int element_repeated_set(strset_t* set, str_t* sptr)
{
	...
	size_t l=get_len(sptr), p=Hash(str, l)%a_slots(set);
...
{
	size_t l=my_strlen(str), p=Hash(str, l)%a_slots(set);
	bucket_t* bucket = arr(set)[p];
...
{
	size_t h = Hash(elem, l)%a_slots(set);
