...
static array_t* empty = NULL;

void destroy_empty_set(void);
void destroy_empty_set(void)
{
	if(empty != NULL)
		array_del(empty);
...
const array_t* empty_set(void)
{
	if(empty == NULL)
	{
		atexit(destroy_empty_set);
		empty = array_new(0);
	}
	return empty;
}
...
array_t* get_states(automata_t* automata)
{
	return automata->states;
}

strset_t* get_alphabet(automata_t* automata)
{
	return automata->alphabet;
}

void* get_initial_state(automata_t* automata)
{
	return automata->initial_state;
}

array_t* get_final_states(automata_t* automata)
{
